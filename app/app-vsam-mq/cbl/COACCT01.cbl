...
011800                                                                  01250000
      ...
      *ACCOUNT RECORD LAYOUT                                            01263107
       COPY CVACT01Y.                                                   01263207
                                                                        01263307
      *SECURITY: APPROVED QUEUE NAMES TABLE                             01263400
       01 WS-APPROVED-QUEUES.                                           01263500
          05 WS-APPROVED-QUEUE-COUNT       PIC 9(02) VALUE 4.           01263600
          05 WS-APPROVED-QUEUE-TABLE.                                   01263700
             10 FILLER PIC X(48) VALUE 'CARD.DEMO.REPLY.ACCT'.          01263800
             10 FILLER PIC X(48) VALUE 'CARD.DEMO.ERROR'.               01263900
             10 FILLER PIC X(48) VALUE 'CARD.DEMO.REQUEST.ACCT'.        01264000
             10 FILLER PIC X(48) VALUE 'CARD.DEMO.INPUT.ACCT'.          01264100
          05 WS-APPROVED-QUEUE-ENTRY REDEFINES                          01264200
             WS-APPROVED-QUEUE-TABLE                                    01264300
             OCCURS 4 TIMES                                             01264400
             INDEXED BY QUEUE-IDX           PIC X(48).                  01264500
                                                                        01264600
       01 WS-QUEUE-VALIDATION.                                          01264700
          05 WS-QUEUE-VALID-FLAG           PIC X(01) VALUE 'N'.         01264800
             88 QUEUE-IS-VALID             VALUE 'Y'.                   01264900
          05 WS-QUEUE-TO-VALIDATE          PIC X(48).                   01265000
                                                                        01265100
011900                                                                  01265200
...
016201       PERFORM 2000-VALIDATE-QUEUE-NAME                           01328108
016202       IF NOT QUEUE-IS-VALID                                      01328208
016203          MOVE 'INVALID QUEUE NAME' TO MQ-ERROR-PARA              01328308
016204          MOVE INPUT-QUEUE-NAME TO MQ-APPL-QUEUE-NAME             01328408
016205          MOVE 'QUEUE NOT IN APPROVED LIST'                       01328508
016206                                TO MQ-APPL-RETURN-MESSAGE         01328608
016207          PERFORM 9000-ERROR                                      01328708
016208          PERFORM 8000-TERMINATION                                01328808
016209       END-IF                                                     01328908
016300       MOVE 'CARD.DEMO.REPLY.ACCT' TO REPLY-QUEUE-NAME            01329008
016301       PERFORM 2000-VALIDATE-QUEUE-NAME                           01329108
016302       IF NOT QUEUE-IS-VALID                                      01329208
016303          MOVE 'INVALID QUEUE NAME' TO MQ-ERROR-PARA              01329308
016304          MOVE REPLY-QUEUE-NAME TO MQ-APPL-QUEUE-NAME             01329408
016305          MOVE 'QUEUE NOT IN APPROVED LIST'                       01329508
016306                                TO MQ-APPL-RETURN-MESSAGE         01329608
016307          PERFORM 9000-ERROR                                      01329708
016308          PERFORM 8000-TERMINATION                                01329808
016309       END-IF                                                     01329908
016400     ELSE                                                         01330007
...
015000     .                                                            01570000
015100                                                                  01580000
016000 2000-VALIDATE-QUEUE-NAME.                                        01581000
016100* VALIDATE QUEUE NAME AGAINST APPROVED LIST                       01582000
016200     MOVE 'N' TO WS-QUEUE-VALID-FLAG                              01583000
016300     MOVE WS-QUEUE-TO-VALIDATE TO WS-QUEUE-TO-VALIDATE            01584000
016400     PERFORM VARYING QUEUE-IDX FROM 1 BY 1                        01585000
016500             UNTIL QUEUE-IDX > WS-APPROVED-QUEUE-COUNT            01586000
016600                OR QUEUE-IS-VALID                                 01587000
016700        IF WS-QUEUE-TO-VALIDATE =                                 01588000
016800           WS-APPROVED-QUEUE-ENTRY(QUEUE-IDX)                     01589000
016900           SET QUEUE-IS-VALID TO TRUE                             01590000
017000        END-IF                                                    01591000
017100     END-PERFORM                                                  01592000
017200     .                                                            01593000
017300                                                                  01594000
...
018000                                                                  01870000
018001******************************************************************01871000
018002* SECURITY NOTE: MQ CHANNEL ENCRYPTION REQUIRED                  *01872000
018003* CONFIGURE MQ CHANNEL WITH SSL/TLS (SSLCIPH, SSLCAUTH)          *01873000
018004* SET MQMD-USERIDENTIFIER FOR MESSAGE AUTHENTICATION             *01874000
018005* ENABLE MESSAGE SIGNING VIA MQMD-ACCOUNTINGTOKEN                *01875000
018006* CONSULT MQ ADMIN TO ENABLE CHANNEL SECURITY EXITS              *01876000
018007******************************************************************01877000
018400                                                                  01910000
018500     MOVE SPACES           TO MQOD-OBJECTQMGRNAME                 01920007
018600     MOVE INPUT-QUEUE-NAME TO MQOD-OBJECTNAME                     01930007
018601     MOVE WS-QUEUE-TO-VALIDATE TO INPUT-QUEUE-NAME                01930108
018700                                                                  01940000
...
021700                                                                  02240000
021701******************************************************************02241000
021702* SECURITY NOTE: MQ CHANNEL ENCRYPTION REQUIRED                  *02242000
021703* CONFIGURE MQ CHANNEL WITH SSL/TLS (SSLCIPH, SSLCAUTH)          *02243000
021704* SET MQMD-USERIDENTIFIER FOR MESSAGE AUTHENTICATION             *02244000
021705* ENABLE MESSAGE SIGNING VIA MQMD-ACCOUNTINGTOKEN                *02245000
021706* CONSULT MQ ADMIN TO ENABLE CHANNEL SECURITY EXITS              *02246000
021707******************************************************************02247000
022100                                                                  02280000
022200     MOVE SPACES            TO MQOD-OBJECTQMGRNAME                02290007
022300     MOVE REPLY-QUEUE-NAME  TO MQOD-OBJECTNAME                    02300007
022301     MOVE WS-QUEUE-TO-VALIDATE TO REPLY-QUEUE-NAME                02300108
022400                                                                  02310000
...
025400                                                                  02610000
025401******************************************************************02611000
025402* SECURITY NOTE: MQ CHANNEL ENCRYPTION REQUIRED                  *02612000
025403* CONFIGURE MQ CHANNEL WITH SSL/TLS (SSLCIPH, SSLCAUTH)          *02613000
025404* SET MQMD-USERIDENTIFIER FOR MESSAGE AUTHENTICATION             *02614000
025405* ENABLE MESSAGE SIGNING VIA MQMD-ACCOUNTINGTOKEN                *02615000
025406* CONSULT MQ ADMIN TO ENABLE CHANNEL SECURITY EXITS              *02616000
025407******************************************************************02617000
025800                                                                  02650000
025900     MOVE 'CARD.DEMO.ERROR' TO ERROR-QUEUE-NAME                   02660000
025901     MOVE ERROR-QUEUE-NAME TO WS-QUEUE-TO-VALIDATE                02660100
025902     PERFORM 2000-VALIDATE-QUEUE-NAME                             02660200
025903     IF NOT QUEUE-IS-VALID                                        02660300
025904        MOVE 'INVALID QUEUE NAME' TO MQ-ERROR-PARA                02660400
025905        MOVE ERROR-QUEUE-NAME TO MQ-APPL-QUEUE-NAME               02660500
025906        MOVE 'QUEUE NOT IN APPROVED LIST'                         02660600
025907                              TO MQ-APPL-RETURN-MESSAGE           02660700
025908        DISPLAY MQ-ERR-DISPLAY                                    02660800
025909        PERFORM 8000-TERMINATION                                  02660900
025910     END-IF                                                       02661000
...
030001******************************************************************03071000
030002* SECURITY NOTE: MQ CHANNEL ENCRYPTION REQUIRED                  *03072000
030003* CONFIGURE MQ CHANNEL WITH SSL/TLS (SSLCIPH, SSLCAUTH)          *03073000
030004* SET MQMD-USERIDENTIFIER FOR MESSAGE AUTHENTICATION             *03074000
030005* ENABLE MESSAGE SIGNING VIA MQMD-ACCOUNTINGTOKEN                *03075000
030006* CONSULT MQ ADMIN TO ENABLE CHANNEL SECURITY EXITS              *03076000
030007******************************************************************03077000
...
037300                                                                  03910000
037301******************************************************************03911000
037302* SECURITY NOTE: MQ CHANNEL ENCRYPTION REQUIRED                  *03912000
037303* CONFIGURE MQ CHANNEL WITH SSL/TLS (SSLCIPH, SSLCAUTH)          *03913000
037304* SET MQMD-USERIDENTIFIER FOR MESSAGE AUTHENTICATION             *03914000
037305* ENABLE MESSAGE SIGNING VIA MQMD-ACCOUNTINGTOKEN                *03915000
037306* CONSULT MQ ADMIN TO ENABLE CHANNEL SECURITY EXITS              *03916000
037307******************************************************************03917000
037600                                                                  03940000
...
041300                                                                  04310000
041301******************************************************************04311000
041302* SECURITY NOTE: MQ CHANNEL ENCRYPTION REQUIRED                  *04312000
041303* CONFIGURE MQ CHANNEL WITH SSL/TLS (SSLCIPH, SSLCAUTH)          *04313000
041304* SET MQMD-USERIDENTIFIER FOR MESSAGE AUTHENTICATION             *04314000
041305* ENABLE MESSAGE SIGNING VIA MQMD-ACCOUNTINGTOKEN                *04315000
041306* CONSULT MQ ADMIN TO ENABLE CHANNEL SECURITY EXITS              *04316000
041307******************************************************************04317000
041600                                                                  04340000
