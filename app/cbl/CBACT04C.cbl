******************************************************************
      ...
      ******************************************************************
      * SECURITY NOTE: This batch program requires exclusive access     
      * to the ACCOUNT-FILE (ACCTFILE) to prevent concurrent update     
      * conflicts and lost updates during REWRITE operations.           
      *                                                                 
      * PREREQUISITE: Before executing this job, ensure ACCTFILE is     
      * closed to all online/CICS regions and other batch jobs via:     
      *   - CICS CLOSEFIL command (CEMT SET FILE(ACCTFILE) CLOSED)      
      *   - Job scheduling controls to serialize access                 
      *   - DISP=OLD in JCL to enforce exclusive allocation             
      *                                                                 
      * Failure to ensure exclusive access may result in data integrity 
      * issues including lost updates to account balances.              
      ******************************************************************
      ...
      *****************************************************************         
       PROCEDURE DIVISION USING EXTERNAL-PARMS.                                 
           ...
           DISPLAY '*** SECURITY: ENSURE ACCTFILE HAS EXCLUSIVE ACCESS'.        
           DISPLAY '*** CICS REGIONS MUST HAVE ACCTFILE CLOSED'.                
      ...
      *---------------------------------------------------------------*         
       0300-ACCTFILE-OPEN.                                                      
      *    SECURITY: Account file opened I-O for batch update.                  
      *    This requires exclusive access - CICS must have file closed.         
      *    JCL should specify DISP=OLD to enforce serialization.                
           MOVE 8 TO APPL-RESULT.                                               
           OPEN I-O ACCOUNT-FILE                                                
           IF  ACCTFILE-STATUS = '00'                                           
               MOVE 0 TO APPL-RESULT                                            
               DISPLAY '*** ACCTFILE OPENED I-O - EXCLUSIVE ACCESS OK'          
           ELSE                                                                 
               MOVE 12 TO APPL-RESULT                                           
           END-IF                                                               
           IF  APPL-AOK                                                         
           ...
           ELSE                                                                 
               ...
               DISPLAY '*** CHECK IF CICS HAS FILE OPEN (CONFLICT)'             
      ...
      * SECURITY: REWRITE assumes exclusive file access (I-O mode).             
      * Concurrent access from CICS or other batch jobs will cause               
      * lost updates. Ensure ACCTFILE is closed in CICS before running.         
      ...
      *---------------------------------------------------------------*         
       9300-ACCTFILE-CLOSE.                                                     
           ...
           IF  ACCTFILE-STATUS  = '00'                                          
               MOVE 0 TO APPL-RESULT                                            
               DISPLAY '*** ACCTFILE CLOSED - CICS CAN REOPEN NOW'              
           ELSE                                                                 
               MOVE 12 TO APPL-RESULT                                           
           END-IF                                                               
           IF  APPL-AOK                                                         
